...
class RoomApiList(mixins.ListModelMixin, generics.GenericAPIView):
    ...
    def filter_queryset(self, queryset):
        ...
        def room_available_during_period(room, arrive, depart):
            ...
            return not zero_quantity_dates

        qs = queryset.filter(location__slug=self.kwargs["location_slug"])
        params = self.request.query_params.dict()
        if params:
            arrive = dateutil.parser(params["arrive"]).date
            depart = dateutil.parser(params["depart"]).date
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not self.location.rooms_with_future_capacity():
            ...
            return HttpResponseRedirect(
                reverse("location_detail", args=(self.location.slug,))
            )
...
def BookingDetail(request, booking_id, location_slug):
    ...
    if (
        ...
        messages.add_message(
            request, messages.INFO, "You do not have permission to view that booking"
        )
        return HttpResponseRedirect("/404")
...
def BookingReceipt(request, location_slug, booking_id):
    ...
    return render(
        ...
        {"receipt_html": receipt_html, "booking": booking, "location": location},
    )
    ...
    if request.user.is_authenticated and request.user == booking.use.user:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                booking.save()
                form.save()
                if not booking.is_pending() and (
                    booking.use.arrive != original_arrive
                    or booking.use.depart != original_depart
                    or booking.use.resource != original_room
                ):
...
def BookingConfirm(request, booking_id, location_slug):
    ...
    if not booking.use.user.profile.stripe_customer_id:
        messages.add_message(
            request,
            messages.INFO,
            "Please enter payment information to confirm your booking.",
        )
    else:
        try:
